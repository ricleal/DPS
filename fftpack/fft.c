 ...
 ********************************************************************
 ...
 NOTES from GAMS Users about FFT.C in FFTPACK

 -----------------------------------------------------------------------------
 Contributed by R. Boisvert, boisvert@nist.gov, 12 Dec 1997
 -----------------------------------------------------------------------------

 1. The user-level routines appear to be:
 ...
 __ogg_fdcosqi == initialization
 __ogg_fdcosqf == forward transform
 __ogg_fdcosqb == backward transform
 ...
 r == sequence to be transformed (input)
 == transformed sequence (output)
 wsave == work array of length 3n (allocated by caller)
 ifac == work array of length 15 (allocated by caller)


 2. The include files ogg.h and fft.h are not necessary.  Instead, just
 include the following

 #define STIN static

 3. The details of the transformation computed can be found in the
 documentation for the Fortran package FFTPACK developed by Paul
 Swarztrauber of NCAR.  The Fortran subroutine name is obtained
 by removing the prefix __ogg_fd from the C procedure names given
 above.
...
STIN void dradf4(int ido, int l1, float *cc, float *ch, float *wa1, float *wa2,
		float *wa3) {
...
STIN void dradb3(int ido, int l1, float *cc, float *ch, float *wa1, float *wa2) {
	static float taur = -.5;
...
STIN void dradb4(int ido, int l1, float *cc, float *ch, float *wa1, float *wa2,
		float *wa3) {
	static float sqrt2 = 1.4142135623730950488016887242097;

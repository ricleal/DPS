...
scitbx::vec3<double> dps_fractional_to_miller(const scitbx::vec3<double>& p) {
	return scitbx::vec3<double>(dps_ai_round(p[0]), dps_ai_round(p[1]),
			dps_ai_round(p[2]));
}

pd::dps_core::dps_core() :
		granularity(5), amax(100), outliers_marked(false) {
}
...
void pd::dps_core::setMaxcell(const double& a) {
	amax = a;
}

pd::fftptr pd::dps_core::fft_factory(const pd::Direction& angle) const {
	fftptr rv(new Directional_FFT(angle, xyzdata, granularity, amax));
	return rv;
}

void pd::dps_core::setSolutions(af::shared<pd::Direction> nwsoln) {
	//sort
	...
	if (hemisphere_solutions.size() == 0) {
		return;
	}
	...
	for (sztype i = hemisphere_solutions.size() - 1; i > 0; --i) {
		for (sztype j = 0; j < i; ++j) {
			if (hemisphere_solutions[i].is_nearly_collinear(
					hemisphere_solutions[j])) {
				hemisphere_solutions[i].kval = 0.0; //mark for later removal
				break;
			}
		}
	}
	first = hemisphere_solutions.begin();
	last = hemisphere_solutions.end();
	std::sort<Ran, kvalcmp>(first, last, kvalcmp());
...
af::shared<pd::Direction> pd::dps_core::getSolutions() const {
...
void pd::dps_core::classify_spots() {
	reset_spots();
}

void pd::dps_core::reset_spots() {
...
double pd::dps_core::rmsdev() const {
	int N = 0;
	double sumsq = 0;
...
pd::pointlistmm pd::dps_core::observed() const {
...
pd::hkllistmm pd::dps_core::hklobserved() const {
	return hklobserved(observed());
}

pd::hkllistmm pd::dps_core::hklobserved(const pointlistmm & spots) const {
